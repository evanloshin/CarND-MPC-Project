...
// Setup cost weighting factors
double cte_weight = 2000;
double epsi_weight = 2000;
double v_weight = 5;
double delta_weight = 5;
double a_weight = 5;
double delta_jerk_weight = 200;
double a_jerk_weight = 10;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += delta_jerk_weight * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += a_jerk_weight * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
    
    // Add constraints to fg
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //Also return future MPC predicted coordinates for sim visualization
  for (int i = 0; i < N - 1; i++) {
